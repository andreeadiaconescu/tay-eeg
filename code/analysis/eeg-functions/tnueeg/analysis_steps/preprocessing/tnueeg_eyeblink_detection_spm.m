...
if isfield(options.eeg.preproc, 'eyeblinkdetection')
    % for backward compatibility
    switch options.eeg.preproc.eyeblinkdetection
        case 'sdThresholding'
            S.methods.fun = 'sdthresh';
            S.methods.settings.threshold = options.eeg.preproc.eyeblinkthreshold; % SD of EOG amplitude
        ...
        case 'ampThresholding'
            ...
            S.methods.settings.threshold = options.eeg.preproc.eyeblinkthreshold; % EOG amplitude in uV
            S.methods.settings.excwin = 1000;
            S.prefix = 'b';
        case 'ampThreshUnfiltered'       
            S.methods.fun = 'threshchan';
            S.methods.settings.threshold = options.eeg.preproc.eyeblinkthreshold; % in mV (applied to EOG amplitude)
            S.methods.settings.excwin = 1000;
    ...
    switch options.eeg.preproc.eyeCorrMethod 
        case 'reject'
            ...
            S.windowOfInterestInSamples = options.eeg.preproc.windowForEyeblinkdetection;     
        case {'ssp', 'berg'}
            % here, we include all EB events, even very large ones that might be other artefacts -
...
fprintf('\nPerforming eye blink detection using a threshold of %s standard deviations.\n', ...
    num2str(options.eeg.preproc.eyeblinkthreshold));
