function exp_MMN_scanner_instructions_with_tone_test(subject, hand, scanner_mode)
...
% Wait for key press
if scanner_mode == 1
    [~,t] = waitkeydown(inf,[MMN.keys.left,MMN.keys.right,MMN.keys.down,MMN.keys.up]);
else
    [~,t] = waitkeydown(inf);
end
...
if scanner_mode == 1
    waitkeydown(inf,[MMN.keys.left,MMN.keys.right,MMN.keys.down,MMN.keys.up]);
else
    waitkeydown(inf);
end
